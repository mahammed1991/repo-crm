...
def upload(request):  # ajax upload file to a question or answer
    ...
    try:
        ...
        if not request.user.user.can_upload_files():
...
def ask(request):
    ...
    if request.POST:
        if request.session.pop('reviewing_pending_data', False):
            ...
            if form.is_valid():
                if request.user.is_authenticated() and request.user.user.email_valid_and_can_ask():
                    ask_action = AskAction(user=request.user.user, ip=request.META['REMOTE_ADDR']).save(data=form.cleaned_data)
...
def convert_to_question(request, id):
    ...
    if not user.user.can_convert_to_question(node):
...
def edit_question(request, id):
    ...
    if question.nis.deleted and not request.user.user.can_view_deleted_post(question):
        raise Http404
    if request.user.user.can_edit_post(question):
        return _edit_question(request, question)
    elif request.user.user.can_retag_questions():
...
def _retag_question(request, question):
    if request.method == 'POST':
        ...
        if form.is_valid():
            if form.has_changed():
                RetagAction(user=request.user.user, node=question, ip=request.META['REMOTE_ADDR']).save(data=dict(tagnames=form.cleaned_data['tags']))
...
def _edit_question(request, question, template='question_edit.html', summary='', action_class=ReviseAction,
    ...
    if request.method == 'POST':
        ...
        if 'select_revision' in request.POST:
            form = EditQuestionForm(question, request.user.user, revision)
        else:
            form = EditQuestionForm(question, request.user.user, revision, data=request.POST)

        if not 'select_revision' in request.POST and form.is_valid():
            if form.has_changed():
                action = action_class(user=request.user.user, node=question, ip=request.META['REMOTE_ADDR']).save(data=form.cleaned_data)

                if settings.WIKI_ON:
                    if request.POST.get('wiki', False) and not question.nis.wiki:
                        question.nstate.wiki = action
                    elif question.nis.wiki and (not request.POST.get('wiki', False)) and request.user.user.can_cancel_wiki(question):
                        question.nstate.wiki = None
            else:
                if not revision == question.active_revision:
                    if allow_rollback:
                        RollbackAction(user=request.user.user, node=question).save(data=dict(activate=revision))
                    else:
                        pass
        ...
        form = EditQuestionForm(question, request.user.user, initial={'summary': summary})
...
def edit_answer(request, id):
    ...
    if answer.deleted and not request.user.can_view_deleted_post(answer):
        raise Http404
    elif not request.user.user.can_edit_post(answer):
        raise Http404
    ...
    if request.method == "POST":
        ...
        if 'select_revision' in request.POST:
            form = EditAnswerForm(answer, request.user.user, revision)
        else:
            form = EditAnswerForm(answer, request.user.user, revision, data=request.POST)

        if not 'select_revision' in request.POST and form.is_valid():
            if form.has_changed():
                action = ReviseAction(user=request.user.user, node=answer, ip=request.META['REMOTE_ADDR']).save(data=form.cleaned_data)

                if settings.WIKI_ON:
                    if request.POST.get('wiki', False) and not answer.nis.wiki:
                        answer.nstate.wiki = action
                    elif answer.nis.wiki and (not request.POST.get('wiki', False)) and request.user.user.can_cancel_wiki(answer):
                        answer.nstate.wiki = None
            else:
                if not revision == answer.active_revision:
                    RollbackAction(user=request.user.user, node=answer, ip=request.META['REMOTE_ADDR']).save(data=dict(activate=revision))

            return HttpResponseRedirect(answer.get_absolute_url())

    else:
        revision_form = RevisionForm(answer)
        form = EditAnswerForm(answer, request.user.user)
