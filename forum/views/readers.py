...
def index(request):
    try:
        ...
        usr.profile_image_url = request.session['profile_image'] if 'profile_image' in request.session else ''
        usr.save()
...
def update_question_view_times(request, question):
    ...
    if (not last_seen) or (last_seen < question.last_activity_at):
        QuestionViewAction(question, request.user.user, ip=request.META['REMOTE_ADDR']).save()
...
def question(request, id, slug='', answer=None):
    ...
    if answer:
        ...
        if (question.nis.deleted and not request.user.user.can_view_deleted_post(question)) or answer.question != question:
            raise Http404
        if answer.marked:
    ...
    answers = request.user.user.get_visible_answers(question)

    update_question_view_times(request, question)

    if request.user.is_authenticated():
        try:
            subscription = QuestionSubscription.objects.get(question=question, user=request.user.user)
        except:
            subscription = False
    else:
        subscription = False
