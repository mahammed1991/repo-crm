...
def mark_favorite(request, id):
    ...
    try:
        favorite = FavoriteAction.objects.get(canceled=False, node=node, user=request.user.user)
        favorite.cancel(ip=request.META['REMOTE_ADDR'])
        added = False
    except ObjectDoesNotExist:
        FavoriteAction(node=node, user=request.user.user, ip=request.META['REMOTE_ADDR']).save()
        added = True
...
def convert_comment_to_answer(request, id):
    ...
    if not user.user.can_convert_comment_to_answer(comment):
...
def mark_tag(request, tag=None, **kwargs):  # tagging system
    ...
    ts = MarkedTag.objects.filter(user=request.user.user, tag__name=tag)
    if action == 'remove':
        ...
        if len(ts) == 0:
            try:
                ...
                mt = MarkedTag(user=request.user.user, reason=reason, tag=t)
                mt.save()
            except:
                pass
        else:
...
def award_points(request, user_id, answer_id):
    ...
    if not request.POST:
        ...
        AwardPointsAction(user=user, node=answer, extra=extra).save(data=dict(value=points, affected=awarded_user))
