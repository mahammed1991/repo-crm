...
class PaginatorContext(object):
    ...
    def preferences(self, request):
        if request.user.is_authenticated():
            if request.user.user.prop.pagination:
                preferences = request.user.user.prop.pagination.get(self.id, {})
            else:
                preferences = {}
        else:
    ...
    def set_preferences(self, request, preferences):
        if request.user.is_authenticated():
            all_preferences = request.user.user.prop.pagination or {}
            all_preferences[self.id] = preferences
            request.user.user.prop.pagination = all_preferences
        else:
    ...
    def sort(self, request, session_prefs=None):
        ...
        sort = None
        sticky = request.user.is_authenticated() and request.user.user.prop.preferences and request.user.user.prop.preferences.get('sticky_sorts', False)
