...
$("#filter_report_type").change(function() {
  ...
  $('#location_split').attr("checked", false);
  $('#default_report_chart_view').hide();
  $('#get_report').show();
  $('#report_chart_view').hide();
  $('#report_type').hide();
  $("#download").prop('disabled', false);
...
$("#get_report").click(function(){
  ...
  if($('#location_split').is(":checked")){
    ...
    if(isError){
      ...
      $('.main').trigger('click');
      $('#columnchart').show();
      $('#lead_status_table').show();
      $('#get_report').hide();
    }
});

// Get values for the selected report view
//$('#report_type').change(function(){
  $(document).on('click', '.diff-chart-view', function(){
  $("#download").prop('disabled', false);
    $('#form_ldap_id').prop("value",window.current_ldap);
    var isError = false;
    var dataString = {}
    ...
    var selectedTimeline = $("#filter_timeline").val();
    //var selectedRegion = $('#filter_region').val();
    dataString['report_view_type'] = $(this).val();
    // Get report type details
    if(!selectedReportType){
        var errMsg = "Please select report type";
          showErrorMessage(errMsg);
  ...
  if($('#location_split').is(":checked")){
    ...
    if(isError){
      ...
      // Ajax call for get reports
      callAjaxforSelectedviewtype(dataString);
    }
});


// Ajax call for selected view type
function callAjaxforSelectedviewtype(dataString){
  $.ajax({
    url: "/reports/get_selected_view_type",
    data: dataString,
    type: 'GET',
    dataType: "JSON",
    success: function(data){
      ...
      $('#week-on-week').hide();
      if(data['report_split_detail']['program_report']){
        $('#week-on-week').show();
        programViewReport(data['report_split_detail']['program_report']);
      }
      if(data['report_split_detail']['region_report']){
        $('#week-on-week').show();
        regionViewReport(data['report_split_detail']['region_report'])
      }
      if (data['report_view_type'] == 'lead_summary_status'){
        $('.c').show();
        $('#view_reports').show();
        $('#lead-status-header').show();
        $('#lead_status_table').show();
        $('#columnchart').show();
        $('#piechart-table').hide();
        $('#linechart').hide();
        $('#task-type-header').hide();
        $('#line_chart_table').hide();
        drawColumnChart(data['lead_status_summary']);
        displayLeadStatusTable(data['lead_status_summary']);
      }else if (data['report_view_type'] == 'task_type_analysis'){
        $('.c').hide();
        $('#columnchart').hide();
        $('#view_reports').show();
        $('#lead-status-header').hide();
        $('#linechart').hide();
        $('#line_chart_table').hide();
        $('#piechart-table').show();
        $('#task-type-header').show();
        drawPieChart(data['pie_chart_dict']);
        newTable(data['table_header'], data['lead_status_analysis_table_grp']);
      }else{
        $('.c').hide();
        $('#columnchart').hide();
        $('#lead-status-header').hide();
        $('#view_reports').show();
        $('#linechart').show();
        $('#line_chart_table').show();
        $('#piechart-table').hide();
        $('#task-type-header').hide();
        $('#week-on-week').show();
        drawLineChart(data['timeline_chart_details']);
        if (data['report_timeline'][0] == "this_quarter"){
          customeTimeLineChartTable(data['timeline_chart_details'], data['sort_keys'], 'Months');
        }
        else if(data['report_timeline'].length > 1){
          displayLineChartTable(data['timeline_chart_details']);
        }
        else{
          customeTimeLineChartTable(data['timeline_chart_details'], data['sort_keys'], 'Weeks');
        }
      }
    },
...
function callAjax(dataString){
  ...
  $.ajax({
        url: "/reports/get_selected_new_reports",
        data: dataString,
        type: 'GET',
        dataType: "json",
        success: function(data) {
          ...
          $('#lead_status_table').empty();
          drawColumnChart(data['reports']);
          displayLeadStatusTable(data['reports']);
